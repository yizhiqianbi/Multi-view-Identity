...
class BasePipeline(torch.nn.Module):
    ...
    def generate_noise(self, shape, seed=None, rand_device="cpu", rand_torch_dtype=torch.float32, device=None, torch_dtype=None):
        ...
        # CPU doesn't support bf16, use float32 for generation then convert
        target_dtype = torch_dtype or self.torch_dtype
        target_device = device or self.device

        # Use float32 on CPU if target is bf16 (CPU doesn't support bf16 randn)
        if rand_device == "cpu" and target_dtype == torch.bfloat16:
            gen_dtype = torch.float32
        else:
            gen_dtype = rand_torch_dtype

        generator = None if seed is None else torch.Generator(rand_device).manual_seed(seed)
        noise = torch.randn(shape, generator=generator, device=rand_device, dtype=gen_dtype)
        noise = noise.to(dtype=target_dtype, device=target_device)
        return noise
    ...
    def download_if_necessary(self, use_usp=False):
        if self.path is None:
            ...
            if not skip_download:
                ...
                # print(os.path.join(self.local_model_path, self.model_id))
                # print(allow_file_pattern)
                snapshot_download(
                    self.model_id,
