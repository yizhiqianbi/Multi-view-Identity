...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, num_heads: int, attn_mask: Optional[torch.Tensor] = None, compatibility_mode=False):
...
def sinusoidal_embedding_1d(dim, position):
    ...
    if position.dim() == 1:
        # 1D case: use outer product
        sinusoid = torch.outer(position.type(torch.float64), freqs)
    else:
        # 2D case: (batch, tokens) -> (batch, tokens, dim//2)
        sinusoid = position.type(torch.float64).unsqueeze(-1) * freqs.unsqueeze(0).unsqueeze(0)
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs, attn_mask: Optional[torch.Tensor] = None):
