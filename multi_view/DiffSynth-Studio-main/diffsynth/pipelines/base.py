...
class BasePipeline(torch.nn.Module):
    ...
    def generate_noise(self, shape, seed=None, device="cpu", dtype=torch.float16):
        ...
        # Use float32 for generation on CPU (bf16 not supported on CPU), then convert
        gen_dtype = dtype if (device != "cpu" or dtype != torch.bfloat16) else torch.float32
        noise = torch.randn(shape, generator=generator, device=device, dtype=gen_dtype)
        # Convert to target dtype and move to GPU if needed
        if gen_dtype != dtype:
